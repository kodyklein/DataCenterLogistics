...
export interface BuildingSettings {
  ...
  mechEvery: number;
  showLandscaping: boolean; // kept for compatibility with capabilities.json (not used in render right now)
...
export const DefaultSettings: BuildingSettings = {
  ...
  mechEvery: 4,
  showLandscaping: true, // kept but currently not used (landscaping code commented out in visual.ts)
  windowDensity: 1.0,
  ...
  // Slightly reduced default for footer header (you can still override in Format pane)
  textHdr2: { size: 14, bold: true,  italic: false, underline: false, align: "center", wrap: false },

  textBody: { size: 12, bold: false, italic: false, underline: false, align: "center", wrap: false },

  // Warning defaults: smaller + not bold by default (still format-pane controllable)
  textWarn: { size: 10, bold: false, italic: false, underline: false, align: "center", wrap: true }
};

export function getSettings(dataView?: powerbi.DataView): BuildingSettings {
  ...
  const mechEvery =
    (typeof b.mechEvery === "number" && b.mechEvery >= 2)
      ? b.mechEvery
      : DefaultSettings.mechEvery;

  const showLandscaping =
    (typeof b.showLandscaping === "boolean" ? b.showLandscaping : DefaultSettings.showLandscaping);

  const windowDensity =
    (typeof b.windowDensity === "number" && b.windowDensity > 0)
      ? b.windowDensity
      : DefaultSettings.windowDensity;
  ...
  const readStyle = (prefix: string, fallback: TextStyle): TextStyle => {
    const size =
      (typeof t[`${prefix}_size`] === "number" && t[`${prefix}_size`] > 0) ? t[`${prefix}_size`] : fallback.size;

    const bold =
      (typeof t[`${prefix}_bold`] === "boolean") ? t[`${prefix}_bold`] : fallback.bold;

    const italic =
      (typeof t[`${prefix}_italic`] === "boolean") ? t[`${prefix}_italic`] : fallback.italic;

    const underline =
      (typeof t[`${prefix}_underline`] === "boolean") ? t[`${prefix}_underline`] : fallback.underline;

    const align = readAlign(t[`${prefix}_align`], fallback.align);

    const wrap =
      (typeof t[`${prefix}_wrap`] === "boolean") ? t[`${prefix}_wrap`] : fallback.wrap;
